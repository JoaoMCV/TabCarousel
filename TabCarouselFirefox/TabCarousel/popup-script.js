...
function atualizarVisibilidade(){
    ...
    if(imagem.classList.contains('btnImgVis')){
        ...
        browser.runtime.sendMessage({action: 'remove', message: this.classList[1]}, ()=>{
        ...
        browser.runtime.sendMessage({action: 'restore', message: this.classList[1]}, ()=>{
            console.log(this.classList[1])
        })
    }
...
function atualizarReload(){
    ...
    if(imagem.classList.contains('reload')){
        ...
        browser.runtime.sendMessage({action: 'stopReload', message: this.classList[1]}, ()=>{
            console.log(this.classList[1])
        })
    }
    else{
        ...
        browser.runtime.sendMessage({action: 'startReload', message: this.classList[1]}, ()=>{
            console.log(this.classList[1])
        })
    }
}
...
function iniciaLista(){
    browser.tabs.query({}, (tabs)=>{
        ...
        for(let i = 0; i< tabs.length;i++){
            ...
            browser.runtime.sendMessage({action: 'getVisibilidade'}, (response)=>{
            ...
            browser.runtime.sendMessage({action: 'getReload'}, (response)=>{
...
extensaobody.onload = ()=>{
    browser.runtime.sendMessage({action: 'getEstado'}, (response)=>{
...
inputSetBtn.addEventListener('click', ()=>{
    browser.runtime.sendMessage({action: 'setVelocidadeCustom', message: input.value}, ()=>{
...
botao.addEventListener('click', ()=>{
    browser.runtime.sendMessage({action: 'start/stop'},(response)=>{
        console.log(response.estado)
        updateButton(response.estado)
    })
})
...
speedSelectDevagar.addEventListener('click', ()=>{
    browser.runtime.sendMessage({action: 'setVelocidadeDevagar'}, ()=>{
...
speedSelectPadrao.addEventListener('click', ()=>{
    browser.runtime.sendMessage({action: 'setVelocidadePadrao'}, ()=>{
...
speedSelectRapido.addEventListener('click', ()=>{
    browser.runtime.sendMessage({action: 'setVelocidadeRapida'}, ()=>{
