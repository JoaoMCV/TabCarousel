...
function alteraIntervalo(){
    ...
    browser.runtime.sendMessage({action:'setIntervalos', index: this.classList.item(1), intervalo: ((parseInt(minuts.value) * 60) + parseInt(seconds.value))*1000})
    
}

function iniciaListaopcoes(){
    browser.tabs.query({}, (tabs)=>{
        browser.runtime.sendMessage({action:'getIntervalo'}, (response)=>{
            intervaloAtual = response.intervalo
            console.log(intervaloAtual)
        
            for(let i = 0; i< tabs.length;i++){
                browser.runtime.sendMessage({action: 'setIntervalo', message:intervaloAtual})
                ...
                browser.runtime.sendMessage({action: 'getIntervalom', message: i},(response)=>{
